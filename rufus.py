#rufus.py - Fully Automated Version with combined.py Integration
...
class RufusInsightsAnalyzer(BaseTool):
    """Custom tool to analyze Rufus questions and customer insights from text file"""
    name: str = "rufus_insights_analyzer"
    description: str = "Analyze Rufus questions and customer insights from a text file"
    args_schema: Type[BaseModel] = RufusInsightsInput
    
    def _run(self, insights_path: str) -> str:
        """Extract and analyze Rufus questions and customer insights"""
        try:
            with open(insights_path, 'r', encoding='utf-8') as f:
                content = f.read()
            
            # Parse the content to extract questions and insights
            lines = content.split('\n')
            ...
            for line in lines:
                ...
                # Identify sections
                if any(keyword in line.lower() for keyword in ['question', 'rufus', 'ask', 'query']):
                    current_section = "questions"
                elif any(keyword in line.lower() for keyword in ['insight', 'feedback', 'customer', 'review']):
                    current_section = "insights"
                elif any(keyword in line.lower() for keyword in ['metric', 'percentage', '%', 'rating', 'score']):
                    current_section = "metrics"
                
                # Extract questions (lines ending with ?)
                if '?' in line:
                    analysis_results["rufus_questions"].append(line)
                
                # Extract metrics (lines with percentages or numbers)
                if any(char in line for char in ['%', 'rating', 'score']) or any(word in line.lower() for word in ['percent', 'rate', 'average']):
                    analysis_results["metrics"].append(line)
                
                # Extract insights based on sentiment
                if any(negative in line.lower() for negative in ['problem', 'issue', 'complaint', 'difficult', 'hard', 'bad', 'poor']):
                    analysis_results["key_concerns"].append(line)
                elif any(positive in line.lower() for positive in ['good', 'great', 'excellent', 'love', 'perfect', 'satisfied', 'happy']):
                    analysis_results["satisfaction_points"].append(line)
                else:
                    analysis_results["customer_insights"].append(line)
            
            # Create summary
            result = f"""
RUFUS QUESTIONS & CUSTOMER INSIGHTS ANALYSIS
==============================================

RUFUS QUESTIONS IDENTIFIED ({len(analysis_results['rufus_questions'])}):
{chr(10).join(f"• {q}" for q in analysis_results['rufus_questions'])}

METRICS & STATISTICS ({len(analysis_results['metrics'])}):
{chr(10).join(f"• {m}" for m in analysis_results['metrics'])}

KEY CUSTOMER CONCERNS ({len(analysis_results['key_concerns'])}):
{chr(10).join(f"• {c}" for c in analysis_results['key_concerns'])}

SATISFACTION POINTS ({len(analysis_results['satisfaction_points'])}):
{chr(10).join(f"• {s}" for s in analysis_results['satisfaction_points'])}

OTHER INSIGHTS ({len(analysis_results['customer_insights'])}):
{chr(10).join(f"• {i}" for i in analysis_results['customer_insights'])}
...
"""
            ...
            return f"Error analyzing insights file {insights_path}: {str(e)}"

# Create the custom tool instances
csv_analyzer = ReliableCSVAnalyzer()
rufus_analyzer = RufusInsightsAnalyzer()
...
review_analyst = Agent(
    ...
    tools=[csv_analyzer, rufus_analyzer]
)
...
def generate_amazon_images_with_rufus(product_name: str, product_id: str, your_csv_path: str, competitor_csv_path: str, rufus_insights_path: str, variants: str):
...
def automated_scrape_and_analyze(product_name, my_product_id, competitor_product_id, variants=""):
    """
    ...
    try:
        ...
        my_reviews = reviews_scraper.scrape_reviews(my_product_id, is_my_product=True, max_pages=5)
        if my_reviews:
        ...
        competitor_reviews = reviews_scraper.scrape_reviews(competitor_product_id, is_my_product=False, max_pages=5)
...
def main():
    """
    Main function - only requires 3-4 simple inputs, everything else is automated
    """
    print("\n🎯 FULLY AUTOMATED AMAZON LISTING IMAGE GENERATOR")
    print("With Rufus Questions + Customer Insights + Review Analysis")
    print("=" * 80)
    print("✨ Just provide 3-4 simple inputs and get 7 unique, customer-focused images!")
    ...
    my_product_id = input("2. Enter your Amazon product ID (from URL): ").strip()
    if not my_product_id:
        print("❌ Your product ID is required!")
        return
    
    competitor_product_id = input("3. Enter competitor Amazon product ID: ").strip()
    ...
    print("\n📝 OPTIONAL INPUT:")
    print("-" * 20)
    variants = input("4. Enter product variants (comma-separated, or press Enter to skip): ").strip()
    ...
    print("   ✅ Scrape Rufus questions from your product page")
    print("   ✅ Scrape customer insights from your product page")
    print("   ✅ Scrape positive reviews from your product")
    print("   ✅ Scrape critical reviews from competitor product")
    print("   ✅ Analyze all data with AI agents")
    print("   ✅ Generate 7 unique, customer-focused Amazon images")
    ...
    try:
        result = automated_scrape_and_analyze(product_name, my_product_id, competitor_product_id, variants)
        ...
        if result:
            ...
            print(f"   • Final images: amazon_images_rufus_integrated_final.txt")
            print("\n✨ Use the final images for your Amazon listing!")
        else:
