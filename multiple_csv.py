...
**Step 2: Analyze COMPETITOR critical reviews CSVs**
For each competitor CSV in the list {competitor_csv_paths} (comma-separated paths), use csv_analyzer to extract and analyze ALL critical review data, but only include reviews with helpful votes. Aggregate pain points across all competitors.

**Step 3: Identify and filter insights**
- From competitors' critical reviews (across all CSVs), identify unique and meaningful customer complaints that are:
  • Specific to the competitors' products or brands
...
- CSV Evidence: Use insights from the comparison table and context analysis to support the content
...
def generate_amazon_images(product_name: str, product_id: str, your_csv_path: str, competitor_csv_paths: list, variants: str, generate_actual_images: bool = False, openai_api_key: str = None):
    ...
    inputs = {
        ...
        "competitor_csv_paths": ','.join(competitor_csv_paths),
        "variants": variants
    }
...
def automated_scrape_and_analyze(product_name, my_product_id, competitor_product_ids: list, variants="", generate_images=False, openai_api_key=None):
    """
    Fully automated function that:
    1. Scrapes positive reviews for your product
    2. Scrapes critical reviews for each competitor product
    ...
    print(f"🏪 Competitor Product IDs: {', '.join(competitor_product_ids)}")
    ...
    try:
        ...
        competitor_csv_paths = []
        # Scrape each competitor's critical reviews
        for competitor_id in competitor_product_ids:
            print(f"\n🔍 Scraping critical reviews for competitor: {competitor_id}")
            competitor_reviews = reviews_scraper.scrape_reviews(competitor_id, is_my_product=False, max_pages=10)
            if competitor_reviews:
                reviews_scraper.save_to_csv(competitor_reviews, competitor_id, is_my_product=False, save_dir=save_dir)
                competitor_csv_path = f"{save_dir}/competitor_critical_reviews_{competitor_id}.csv"
                competitor_csv_paths.append(competitor_csv_path)
                print(f"✅ Competitor reviews saved to: {competitor_csv_path}")
            else:
                print(f"❌ Failed to scrape competitor reviews for {competitor_id}")
    except Exception as e:
    ...
    files_to_check = [(your_csv_path, "Your product reviews CSV")]
    for competitor_csv in competitor_csv_paths:
        files_to_check.append((competitor_csv, f"Competitor reviews CSV ({competitor_csv.split('_')[-1].split('.')[0]})"))
    ...
    try:
        ...
        result = generate_amazon_images(
            ...
            your_csv_path=your_csv_path,
            competitor_csv_paths=competitor_csv_paths,
            variants=variants,
...
def main():
    ...
    print("1. Only competitor product ID(s) (comma-separated; we'll use them for both analysis)")
    print("2. Both your product ID and competitor product ID(s) (comma-separated for competitors)")
    ...
    if id_choice == "1":
        ...
        print("-" * 25)
        competitor_input = input("Enter competitor Amazon product IDs (comma-separated): ").strip()
        if not competitor_input:
            print("❌ At least one competitor product ID is required!")
            return
        competitor_product_ids = [id.strip() for id in competitor_input.split(',') if id.strip()]
        my_product_id = competitor_product_ids[0]  # Use the first competitor as "my" product for analysis
        print(f"✅ Using {competitor_product_ids[0]} as your product ID for positive reviews analysis")
        print(f"✅ Using all provided IDs for critical reviews analysis: {', '.join(competitor_product_ids)}")
        print(" • Positive reviews will be analyzed from the first ID")
        print(" • Critical reviews will be analyzed from all IDs")
    elif id_choice == "2":
        print("\n📝 PRODUCT IDs:")
        print("-" * 15)
        ...
        if not competitor_input:
            ...
            return
        competitor_product_ids = [id.strip() for id in competitor_input.split(',') if id.strip()]
        print(f"✅ Using your product ID: {my_product_id} for positive reviews")
        print(f"✅ Using competitor IDs: {', '.join(competitor_product_ids)} for critical reviews")
    else:
    ...
    print(f"📦 Product: {product_name}")
    if id_choice == "1":
        print(f"🆔 Your Product ID (for positives): {my_product_id}")
        print(f"🏪 Competitor IDs (for criticals): {', '.join(competitor_product_ids)}")
        print("📊 Analysis Mode: Single/Multi product analysis (positives from first, criticals from all)")
    else:
        print(f"🆔 Your Product ID: {my_product_id}")
        print(f"🏪 Competitor IDs: {', '.join(competitor_product_ids)}")
        print("📊 Analysis Mode: Dual/Multi product comparison")
        print(" • Positive reviews from your product")
        print(" • Critical reviews from all competitors")
    ...
    try:
        ...
        if result:
            ...
            for comp_id in competitor_product_ids:
                print(f" • Competitor reviews ({comp_id}): competitor_critical_reviews_{comp_id}.csv")
...
# combined.py remains unchanged as per your request (no modifications needed for scraping logic)